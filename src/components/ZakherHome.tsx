...
const translations = {
  en: {
    ...
    contactForm: { title: "Contact Concierge", subtitle: "How can we assist with your stay?", name: "Full Name *", email: "Email Address *", phone: "WhatsApp / Phone *", language: "Preferred Language (Optional)", requests: "Message/Special Requests *", send: "Send Message", success: "Thanks for your inquiry. A concierge will be in touch shortly.", specifyLanguage: "Please specify language", mandatory: "Indicates a mandatory field", otherLanguage: "Other" },
    night: "/ Night",
  ...
  return (
    <div ref={container} className="min-h-screen bg-[#F9F9F9] text-[#1A1A1A] font-sans selection:bg-[#8BA3A0] selection:text-white">
        ...
        <div className="flex items-center justify-between w-full max-w-full">
          <div className="flex flex-1 items-center gap-8 lg:gap-12">
            <div className="w-auto flex-shrink-0 break-words pr-4">
              <Link href={`/${langState}`} className="font-montserrat text-sm tracking-[0.3em] uppercase font-medium hover:opacity-70 transition-colors block truncate">
      ...
      <section className="relative h-[100vh] w-full overflow-hidden flex flex-col justify-end pb-32 px-8 md:px-16">
        <div className="hero-bg absolute inset-0 z-0 origin-center bg-cover" style={{ backgroundImage: `url(${siteConfig?.hero_image || "/parent-hero.jpg"})`, backgroundPosition: 'center center' }}>
        </div>
        ...
        <div className="relative z-10 text-white max-w-4xl">
          <div>
            ...
            <h1 className="font-cormorant text-6xl md:text-8xl lg:text-9xl font-light leading-none mb-6 drop-shadow-lg">
              {(siteConfig?.site_title || "Grupo Zakher").split(' ').map((word: string, i: number) => (
            ...
            <p className="hero-nav font-montserrat text-sm md:text-base font-light max-w-xl leading-relaxed text-white/90 drop-shadow-md">
              {t.heroSubtitle}
            </p>
          </div>
        </div>
          ...
          >
            ...
            >
              ...
              <iframe
                ...
                src="https://maps.google.com/maps?q=Plaza+de+Bol%C3%ADvar,+Cartagena,+Colombia&t=&z=16&ie=UTF8&iwloc=&output=embed"
              ...
              <div className="absolute bottom-0 left-0 w-full md:w-auto md:max-w-sm md:bottom-6 md:left-6 z-10 p-6 pb-8 md:p-8 bg-white md:rounded-xl shadow-[0_-20px_40px_rgba(0,0,0,0.1)] md:shadow-2xl flex flex-col gap-4 pointer-events-auto">
                ...
                <a 
                  href={`https://www.google.com/maps/search/?api=1&query=Plaza+de+Bolivar,+Cartagena,+Colombia`}
                  target="_blank"
